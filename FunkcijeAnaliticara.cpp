...
void filtrirajPoKupcu(std::string kupac) {
	...
	std::cout << "Ukupan broj racuna ovog kupca je: " << brojac<<std::endl;
	std::cout << "Sve cijene su u valuti " << vratiValutu()<<std::endl;
}
...
void sortiraj_i_ispisi(ModifikovanDatum* niz, int n) {
	...
	std::cout << std::endl << "Pregled ukupne vrijednosti robe prodate u pojedinim mjesecima:" << std::endl<<std::endl;
	for(int i=0;i<n;i++)
